...
def samples_df(
    ...
    Args:
       ...
       exclude_fields: Optional top-level sample fields to skip when full sample
          loading is required. This is most useful with `.eval` logs when doing
          score-only analysis and large fields such as `events` are not needed.
